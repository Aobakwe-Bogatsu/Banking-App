...
// In Unit4.cpp, simply call getField
AnsiString result = getField(1, " ", ',');
...
void __fastcall TForm1::btnsigninClick(TObject *Sender)
{
	...
	while(!tread->EndOfStream && found == false)
	{
		...
		else if(edtaccountnr->Text == AccountNumber && edtpassword->Text == Pword)
		{
			...
			Form2->lblUserSurname->Caption =  NameandSurname.SubString(NameandSurname.Pos(" "), (NameandSurname.Length() - NameandSurname.Pos(" ")));
